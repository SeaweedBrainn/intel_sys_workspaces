...
| **`intel_sys_hardware`** | `ament_python` | STM32 serial driver bridge (`stm32_bridge`), inverse mecanum velocity controller with `/joint_states` publishing (`mecanum_controller`), sensor launchers, and udev rules. |
...
Starts the entire stack (robot description + STM32 bridge + sensors + localization EKF + Nav2):
...
### 2. Launch 3D Simulation (Gazebo Harmonic + RViz)

Spins up the Gazebo obstacle world, spawns the robot model with Mecanum drive physics, and bridges sensors:

```bash
# Simulation with GUI and RViz2:
