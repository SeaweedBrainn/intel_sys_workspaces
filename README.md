...
│   ├── intel_sys_navigation/   # Mecanum odometry, EKF sensor fusion, Point-LIO & Nav2
│   ├── intel_sys_sim/          # Simulation scaffold for Gazebo / Isaac Sim
...
| **`intel_sys_description`** | `ament_cmake` | Unified `robot.urdf.xacro`, chassis and mecanum wheel STL meshes, `robot_state_publisher`, and one-click `view_robot.launch.py`. |
...
| **`intel_sys_sim`** | `ament_cmake` | Simulation bridge package prepared for Gazebo Harmonic / Isaac Sim. |
...
### 2. Preview 3D Robot Description in RViz
